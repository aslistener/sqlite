...
{
    friend class GSQLiteDriver;
    friend class QSQLiteResultPrivate;
public:
    explicit QSQLiteResult(const GSQLiteDriver* db);
    ~QSQLiteResult();
...
class GSQLiteDriverPrivate : public QSqlDriverPrivate
{
public:
    inline GSQLiteDriverPrivate() : QSqlDriverPrivate(), access(0) { dbmsType = GSQLiteDriver::SQLite; }
    sqlite3 *access;
...
QSQLiteResult::QSQLiteResult(const GSQLiteDriver* db)
    : QSqlCachedResult(db)
{
    d = new QSQLiteResultPrivate(this);
    d->access = db->d_func()->access;
    const_cast<GSQLiteDriverPrivate*>(db->d_func())->results.append(this);
}
...
QSQLiteResult::~QSQLiteResult()
{
    ...
    if (sqlDriver)
        const_cast<GSQLiteDriverPrivate*>(qobject_cast<const GSQLiteDriver *>(sqlDriver)->d_func())->results.removeOne(this);
    d->cleanup();
    delete d;
}
...
GSQLiteDriver::GSQLiteDriver(QObject * parent)
    : QSqlDriver(*new GSQLiteDriverPrivate, parent)
{
}

GSQLiteDriver::GSQLiteDriver(sqlite3 *connection, QObject *parent)
    : QSqlDriver(*new GSQLiteDriverPrivate, parent)
{
    Q_D(GSQLiteDriver);
    d->access = connection;
...
GSQLiteDriver::~GSQLiteDriver()
{
}

bool GSQLiteDriver::hasFeature(DriverFeature f) const
{
    switch (f) {
    case BLOB:
    case Transactions:
...
/*
   ...
   just file names.
*/
bool GSQLiteDriver::open(const QString & db, const QString &, const QString &, const QString &, int, const QString &conOpts)
{
    Q_D(GSQLiteDriver);
    if (isOpen())
        close();
...
QSqlResult *GSQLiteDriver::createResult() const
{
    return new QSQLiteResult(this);
}

bool GSQLiteDriver::beginTransaction()
{
...
bool GSQLiteDriver::commitTransaction()
{
    if (!isOpen() || isOpenError())
        return false;
...
bool GSQLiteDriver::rollbackTransaction()
{
    if (!isOpen() || isOpenError())
        return false;
...
QStringList GSQLiteDriver::tables(QSql::TableType type) const
{
    QStringList res;
    if (!isOpen())
        return res;
...
QSqlIndex GSQLiteDriver::primaryIndex(const QString &tblname) const
{
    if (!isOpen())
        return QSqlIndex();
...
QSqlRecord GSQLiteDriver::record(const QString &tbl) const
{
    if (!isOpen())
        return QSqlRecord();
...
QVariant GSQLiteDriver::handle() const
{
    Q_D(const GSQLiteDriver);
    return QVariant::fromValue(d->access);
}

QString GSQLiteDriver::escapeIdentifier(const QString &identifier, IdentifierType type) const
{
    Q_UNUSED(type);
